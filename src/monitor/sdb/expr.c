...
// int eval(int, int);
// ExprResult check_parentheses(int, int);
// bool check_parentheses(int, int);
// ExprResult eval(int,int);
...
int check_parentheses(int p, int q)
{
  ...
  else if (p < q)
  {
    ...
    for (int i = 0; i < q; i++)
    {
      ...
      if (counter < 0)
      {
        // if more right parenthesis than left parenthesis
        return 0;
      }
      // If the counter becomes zero before reaching the end, it means it is not the outermost parenthesis.
      if (counter == 0 && i < q)
      {
        return 0;
      }
    ...
    if (counter == 0)
    {
      return 1; // 1 to bool true
    }
    else if (counter > 0)
    {
      printf("more right parenthesis than right parenthesis.\n");
      return 0; // if more left parenthesis than right parenthesis
    }
    else if (counter < 0)
    {
      printf("more right parenthesis than right parenthesis.\n");
    ...
    {
      printf("unknow eval function counter happend\n");
      return 0;
    }
  }
  else if (p == q)
  {
    printf("Boundary case: There is only one token, and a single token cannot be surrounded by a pair of parentheses.\n");
    return 0;
  }
  else
  {
    printf("unknow check_parentheses p and q happend.\n");
    return 0;
  }
}
  */
/*
ExprResult check_parentheses(int p, int q)
{
 ExprResult result = {0, false};
 if (p > q)
 {
   // fail
 ...
 else if (p < q)
 {
   ...
   if (counter == 0)
   {
     result.success = true; // 1 to bool true
     return result;
   }
   else if (counter > 0)
   {
     printf("more left parenthesis than right parenthesis.\n");
     return result; // if more left parenthesis than right parenthesis
   }
   else if (counter < 0)
   {
...
bool check_parentheses(int p, int q)
{
  ...
  else if (p < q)
  {
    ...
    if (counter == 0)
    {
      return true; // 1 to bool true
    }
    else if (counter > 0)
    {
      printf("more left parenthesis than right parenthesis.\n");
      return false; // if more left parenthesis than right parenthesis
    }
    else if (counter < 0)
    {
      printf("more right parenthesis than left parenthesis.\n");
