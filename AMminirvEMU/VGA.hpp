...
public:
    ...
    void init() {
        for (int i {0}; i < 256 * 256; i++) vmem[i] {0};
    }
    ...
    void write_word(std::uint32_t addr, std::uint32_t data) {
        ...
        if (offset < 256 * 256) vmem[offset] = data;
    }

    // 必须有这个，否则 sb 指令黑屏
    void write_byte(std::uint32_t addr, std::uint8_t value) {
        std::uint32_t offset = addr - FB_ADDR;
        if (offset < 256 * 256 * 4) {
            ((std::uint8_t*)vmem)[offset] = value;
        }
    }

    void update_screen() {
        AM_GPU_FBDRAW_T draw;
        draw.x {0}; draw.y {0}; draw.w = 256; draw.h = 256;
        draw.pixels = vmem; draw.sync = true;
        ioe_write(AM_GPU_FBDRAW, &draw);
    }

private:
    std::uint32_t vmem[256 * 256];
};
#endif