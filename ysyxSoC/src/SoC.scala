...
class ysyxSoCASIC(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) with DontTouch {
    ...
    debug_csr_mstatus := cpu.module.debug_csr_mstatus
    debug_csr_mtvec := cpu.module.debug_csr_mtvec
    debug_csr_mepc := cpu.module.debug_csr_mepc
    debug_csr_mcause := cpu.module.debug_csr_mcause
    debug_trap_mstatus := cpu.module.debug_trap_mstatus
    debug_trap_mtvec := cpu.module.debug_trap_mtvec
    debug_trap_mepc := cpu.module.debug_trap_mepc
    debug_trap_mcause := cpu.module.debug_trap_mcause
    ...
    perf_mdu_req := cpu.module.perf_mdu_req
    ...
    perf_mdu_wait := cpu.module.perf_mdu_wait
    ...
    perf_dcache_hit := cpu.module.perf_dcache_hit
    val perf_dcache_miss = IO(Output(Bool()))
    perf_dcache_miss := cpu.module.perf_dcache_miss
    val perf_dcache_refill_req = IO(Output(Bool()))
    perf_dcache_refill_req := cpu.module.perf_dcache_refill_req
    val perf_dcache_refill_resp = IO(Output(Bool()))
    perf_dcache_refill_resp := cpu.module.perf_dcache_refill_resp
...
class ysyxSoCFull(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) with DontTouch {
    ...
    Seq(sdram0, sdram1).foreach { s =>
      ...
      s.io.cs  := masic.sdram.cs
      s.io.ras := masic.sdram.ras
      s.io.cas := masic.sdram.cas
      s.io.we  := masic.sdram.we
      s.io.a   := masic.sdram.a
      s.io.ba  := masic.sdram.ba
    }
    ...
    val debug_trap_mepc = IO(Output(UInt(32.W)))
    val debug_trap_mcause = IO(Output(UInt(32.W)))
    debug_csr_mstatus := masic.debug_csr_mstatus
    debug_csr_mtvec := masic.debug_csr_mtvec
    debug_csr_mepc := masic.debug_csr_mepc
    debug_csr_mcause := masic.debug_csr_mcause
    debug_trap_mstatus := masic.debug_trap_mstatus
    debug_trap_mtvec := masic.debug_trap_mtvec
    debug_trap_mepc := masic.debug_trap_mepc
    debug_trap_mcause := masic.debug_trap_mcause
    val perf_mem_waitslot = IO(Output(Bool()))
    ...
    val perf_mdu_req = IO(Output(Bool()))
    perf_mdu_req := masic.perf_mdu_req
    val perf_mdu_done = IO(Output(Bool()))
    ...
    val perf_mdu_wait = IO(Output(Bool()))
    perf_mdu_wait := masic.perf_mdu_wait
    val perf_ifu_stall_pipeline = IO(Output(Bool()))
    ...
    val perf_dcache_hit = IO(Output(Bool()))
    perf_dcache_hit := masic.perf_dcache_hit
    val perf_dcache_miss = IO(Output(Bool()))
    perf_dcache_miss := masic.perf_dcache_miss
    val perf_dcache_refill_req = IO(Output(Bool()))
    perf_dcache_refill_req := masic.perf_dcache_refill_req
    val perf_dcache_refill_resp = IO(Output(Bool()))
    perf_dcache_refill_resp := masic.perf_dcache_refill_resp
    val perf_idu_stall_raw = IO(Output(Bool()))
