...
class AXI4ToAPB(val aFlow: Boolean = true)(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val bid_reg    = RegEnable(aw.bits.id, accept_aw_only || accept_aw_wait)
      val araddr_reg = ar.bits.addr holdUnless accept_read
      val awaddr_reg = aw.bits.addr holdUnless (accept_aw_only || accept_aw_wait)
      val wdata_reg  =  w.bits.data holdUnless (accept_w_only || accept_w_wait)
      val wstrb_reg  =  w.bits.strb holdUnless (accept_w_only || accept_w_wait)
