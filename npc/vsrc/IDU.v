...
module IDU(
    ...
    wire is_I_type = (opcode == `OPCODE_Immediate) ||
                     (opcode == `OPCODE_Immediate_Lxxx) ||
                     (opcode == `OPCODE_Immediate_Bxxx);
    wire is_S_type = (opcode == `OPCODE_Store);
    wire is_B_type = (opcode == `OPCODE_Branch);
    wire is_U_type = (opcode == `OPCODE_UpperImmediate_lui) ||
                     (opcode == `OPCODE_UpperImmediate_auipc);
