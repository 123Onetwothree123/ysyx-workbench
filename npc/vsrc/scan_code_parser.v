...
/*
PS/2键盘扫描码解析器，用来解析PS/2协议中的通码Make Code和断码Break Code，然后输出按键按下状态和当前扫描码还有按下脉冲
PS/2协议说明:
按键按下：发送通码，比如A键通码为0x1C
按键释放：先发送0xF0断码前缀,再发送通码（0xF0, 0x1C）
*/
...
localparam WAIT_F0 = 2'b10;//等待断码状态，已收到F0,等待断码字节，如果下一字节和scan_code一样，那么就说明释放的是同一个键，然后key_pressed=0，然后回 IDLE
//状态寄存器
reg [1:0] state;//当前状态
reg [1:0] next_state;//次态，这是组合逻辑输出
//内部寄存器
...
always @(*) begin
    ...
    case (state)
        ...
        default: next_state = IDLE;  //异常状态，然后就回到空闲
    endcase
end
//状态寄存器，ps2_ready为1时就会更新状态
...
//这个的输出逻辑是用来产生scan_code和key_pressed信号的
// scan_code是在IDLE状态收到通码时锁存的，然后一直保持到下一次按键
// key_pressed则是在IDLE状态置1，然后在WAIT_F0状态匹配断码后置0
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (state)
            ...
            WAIT_F0: begin
                //等待断码状态，在这里先检查断码是否匹配
                if (ps2_data == scan_code)
                    key_pressed <= 1'b0;//如果，假设，这个断码匹配就清除按下标志
                //断码不匹配时不处理(异常情况)
            end
...
//检测key_pressed的上升沿，产生单周期脉冲
//key_pressed_pulse是用来搞触发key_counter计数的，确保按住不放只算一次
always @(posedge clk or negedge rst_n) begin
    if (!rst_n)
