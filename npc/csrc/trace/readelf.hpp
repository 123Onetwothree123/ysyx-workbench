...
/**
 * @file readelf.hpp
 * @brief 供 NPC 追踪与调试工具使用的现代 C++23 ELF 元数据读取器。
 *
 * @details
 * 本模块拥有从 ELF 文件中读取的全部字节，并对外暴露轻量的、无所有权的
 * 名称与表项视图。公共 API 有意设计为面向对象的风格：调用者从路径加载一个
 * `Readelf` 对象，然后从这个不可变缓存中查询或打印信息。加载错误通过
 * `std::expected` 报告，而非进程级状态或 errno 式副作用通道。
 *
 * 所选的 ELF 布局遵循 NPC 构建目标。`NPC_ISA64 == 0` 表示使用 ELF32 类型，
 * `NPC_ISA64 != 0` 表示使用 ELF64 类型。
 */
...
/**
 * @brief 供 NPC 使用的 RAII ELF 读取器与符号缓存。
 *
 * @details
 * `Readelf` 读取 ELF 文件头、节头表、可选的节名字符串表、常规符号表、
 * 符号字符串表，以及派生的函数符号列表。它不依赖 NEMU 辅助函数或全局状态。
 *
...
private:
    ...
    // 缓存的符号字符串表。加载器会额外追加一个尾部 NUL 字节。
    std::vector<char> string_table_{};

    // 缓存的节名字符串表。加载器会额外追加一个尾部 NUL 字节。
