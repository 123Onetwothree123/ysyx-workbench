...
/*
 *  menubox.c -> menubox.cpp
 *
 *  ORIGINAL AUTHOR: Savio Lam (lam836@cs.cuhk.hk)
 *  MODIFIED FOR LINUX KERNEL CONFIG BY: William Roadcap (roadcapw@cfw.com)
 *
 *  Changes by Clifford Wolf (god@clifford.at)
 *  [ 1998-06-13 ] Page-Down bugfix, scroll-saving
 *  [ 1998-06-14 ] crash-safe against broken lxdialog.scrltmp files
 *  02 January 1999, Michael Elizabeth Chastain (mec@shout.net)
 *  Reset 'scroll' to 0 if bogus.
 */
...
{
	auto *menu_item = (char *)malloc(menu_width + 1);

	strncpy(menu_item, item, menu_width - item_x);
	menu_item[menu_width - item_x] = '\0';
	int j = first_alpha(menu_item, "YyNnMmHh");
...
do_resize:
	...
	max_choice = std::min(menu_height, item_count());
	...
	menu_width = width - 6;
	int box_y = height - menu_height - 5;
	int box_x = (width - menu_width) / 2 - 1;
		...
		if (item_count() != 0 &&
			...
			if (key == KEY_UP || key == '-') {
				...
				if ((choice > max_choice - 3) &&
					...
					choice = std::min(choice + 1, max_choice - 1);
