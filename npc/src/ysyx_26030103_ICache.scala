...
class ysyx_26030103_ICache(
  ...
  io.axi.AR.ARLEN := Mux(fetch_addr_reg(31), (WordsPerBlock - 1).U, 0.U)
  ...
  switch(state) {
    is(state_idle) {
      ...
      io.perf_hit  := io.fetch_valid && io.fetch_ready && io.fetch_addr(31) && hit
      io.perf_miss := io.fetch_valid && io.fetch_ready && io.fetch_addr(31) && !hit
      when(io.fetch_valid && io.fetch_ready) {
        ...
        cacheable_reg   := io.fetch_addr(31, 31)
        fetch_offset_reg := blockOffset
        refill_cnt      := 0.U
        when(io.fetch_addr(31) && hit) {
          state := state_resp
        }.otherwise {
    ...
    is(state_refill_req) {
      ...
      io.axi.AR.ARADDR := Mux(fetch_addr_reg(31),
    ...
    is(state_refill_resp) {
      ...
      when(io.axi.R.RVALID && io.axi.R.RREADY) {
        ...
        when(io.axi.R.RLAST || !fetch_addr_reg(31)) {
          when(fetch_addr_reg(31)) { valid(fetch_index_reg) := true.B }
          state := state_resp
        }.otherwise {
          when(refill_cnt < (WordsPerBlock - 1).U) { refill_cnt := refill_cnt + 1.U }
        }
      }
    }
    is(state_resp) {
      io.resp_valid := true.B
      io.resp_data := Mux(fetch_addr_reg(31) && !access_fault_reg,
