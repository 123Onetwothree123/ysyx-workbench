...
class ysyx_26030103(val config: ysyx_26030103_NPCConfig = ysyx_26030103_NPCConfig())
  ...
  // EXU redirect/exception must kill the younger instruction queued at the
  // IDU->EXU boundary, while the current EXU instruction remains visible for
  // this cycle so it can commit and generate the redirect.
  ysyx_26030103_StageConnect(
    idu.io.out,
    exu.io.in,
    lsu.io.FlushIDEX || pipe_flush,
    // A redirect/trap generated by the current EXU item must let that item
    // fire; an older MEM fault must kill it even if EXU is backpressured.
    flushCurrent = lsu.io.FlushIDEX
  )
  // This boundary holds an instruction younger than the active LSU one; only
  // an older MEM fault may discard it.  Do not feed pipe_flush here.
